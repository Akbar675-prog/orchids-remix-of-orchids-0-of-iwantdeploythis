...
export function useModel({
  ...
  // Handle model override from events
  useEffect(() => {
    if (typeof window !== "undefined") {
      const override = localStorage.getItem("visora-model-override")
      if (override) {
        setLocalSelectedModel(override)
        localStorage.removeItem("visora-model-override")
      }
    }
  }, [])
